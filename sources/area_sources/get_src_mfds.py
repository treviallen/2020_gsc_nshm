...
# do all sources except Beaufort-Mackenzie Convergence (BMC)
    ...
    # find BMC and remove from source idx
    bmcidx = where(array(src_code) == 'BMC')
    srcidx = delete(srcidx, bmcidx)
    
###############################################################################
# parse SHEEFs
###############################################################################

# in sheef folder, get sheef files
sheeffiles = listdir_extension(sheeffolder, 'gmtdat')

# expects full, deep and crustal sheefs
for sfile in sheeffiles:
    if sfile.endswith('full.gmtdat'):
        fullsheeffile  = sheeffolder+sfile
    elif sfile.endswith('crust.gmtdat'):
        crustsheeffile = sheeffolder+sfile
    elif sfile.endswith('deep.gmtdat'):
        deepsheeffile  = sheeffolder+sfile

print 'Parsing SHEEF files...'
fullsheef  = parse_sheef(fullsheeffile)
crustsheef = parse_sheef(crustsheeffile)
deepsheef  = parse_sheef(deepsheeffile)
...
for i in srcidx:
    ...
    ###############################################################################
        ...
        new_beta_b[i] = beta
        new_beta_l[i] = beta-sigbeta173
        new_beta_u[i] = beta+sigbeta173
        new_n0_b[i]   = fn0
        new_n0_l[i]   = N0_lo173
        new_n0_u[i]   = N0_up173
...
w.field('BETA_BEST','F', 8, 5)
w.field('BETA_LOWER','F', 8, 5)
w.field('BETA_UPPER','F', 8, 5)
w.field('BETA_FIX','F', 8, 2)
w.field('BETA_FIX_S','F', 8, 2)
w.field('YCOMP','C','30')
w.field('MCOMP','C','30')
w.field('YMAX','F', 8, 2)
w.field('SRC_GMPE','C','100')
w.field('SHEEF_FILE','C','10')
...
for record, shape in zip(records, shapes):
    ...
    if i >= 0:
        w.record(newrec[0], newrec[1], newrec[2], newrec[3], newrec[4], newrec[5], newrec[6], \
                 newrec[7], newrec[8], newrec[9], newrec[10], newrec[11], newrec[12], newrec[13], \
                 new_n0_b[i], new_n0_l[i], new_n0_u[i], new_beta_b[i], new_beta_l[i], new_beta_u[i], newrec[20], \
                 newrec[21], newrec[22], newrec[23], newrec[24], newrec[25], newrec[26])
